...
ots::CaloIERCDQM::CaloIERCDQM(Parameters const &conf)
    : art::EDAnalyzer(conf), conf_(conf()), port_(conf().port()),
      ...
      splineFilename_(conf().spline()), uset0_(conf().uset0()),
  ...
  for (auto pair : timeMap) {
    ...
    if (pair.second.size() > 1) { // there must be at least 2 hits
      if (histo_container->map_h1_dt_singlechan.find(thisID) ==
          histo_container->map_h1_dt_singlechan
              .end()) { // This hist doesn't exist yet
        TString hname = Form("h1_dt_schan_%d", thisID);
        TString htitle = Form(
            "[SAME CHANNEL] dt between hit 0 and hit 1 of sipm %d", thisID);
        histo_container->BookSingleChannel(tfs, thisID, hname.Data(),
                                           htitle.Data(), 600, 10009, 10011);
        // map_h1_dt_singlechan[thisID] =
        // tfs->make<TH1F>(hname,htitle,600,10009,10011);
        TString gname = Form("g_dtevt_schan_%d", thisID);
        TString gtitle = Form(
            "[SAME CHANNEL] dt between hit 0 and hit 1 of sipm %d", thisID);
        histo_container->BookSingleChannelG(tfs, thisID, gname.Data(),
                                            gtitle.Data());
    ...
    for (auto nextpair : timeMap) {
      ...
      if (thisDTCROC ==
        ...
        if (histo_container->map_h1_dt_sameboard.find(idpair) ==
            histo_container->map_h1_dt_sameboard
                .end()) { // This hist doesn't exist yet
          TString hname = Form("map_h1_dt_sameboard_%d_%d", nextID, thisID);
          TString htitle = Form("[SAME BOARD] dt between chan %d and %d (DTC: "
                                "%d, Board: %d, hit 0)",
                                nextChan, thisChan, thisDTC, thisROC);
          histo_container->BookSameBoard(tfs, idpair, hname.Data(),
                                         htitle.Data(), 5000, -50, 50);
          ...
          TString gtitle = Form("[SAME BOARD] dt between chan %d and %d (DTC: "
                                "%d, Board: %d, hit 0)",
                                nextChan, thisChan, thisDTC, thisROC);
          histo_container->BookSameBoardG(tfs, idpair, gname.Data(),
                                          gtitle.Data());
      ...
      if (thisChan == chan) { // Found the channel we want to plot
        if (refID < 0) {      // It's the first one, don't do anything
          ...
          if (histo_container->map_h1_dt_diffboard.find(idpair) ==
              histo_container->map_h1_dt_diffboard
                  .end()) { // This hist doesn't exist yet
            TString hname = Form("map_h1_dt_diffboard_chan%d_%d_%d", thisChan,
                                 thisID, refID);
            TString htitle =
                Form("[DIFFERENT BOARD] dt between (DTC: %d, Board: %d) and "
                     "(DTC: %d, Board: %d) [chan %d, hit 0]",
                     thisDTC, thisROC, refDTC, refROC, thisChan);
            histo_container->BookDiffBoard(tfs, idpair, hname.Data(),
                                           htitle.Data(), 20000, -200, 200);
            // map_h1_dt_diffboard[idpair] =
            // tfs->make<TH1F>(hname,htitle,20000,-200,200);
            TString gname = Form("map_g_dtevt_diffboard_chan%d_%d_%d", thisChan,
                                 thisID, refID);
            TString gtitle =
                Form("[DIFFERENT BOARD] dt between (DTC: %d, Board: %d) and "
                     "(DTC: %d, Board: %d) [chan %d, hit 0]",
                     thisDTC, thisROC, refDTC, refROC, thisChan);
            histo_container->BookDiffBoardG(tfs, idpair, gname.Data(),
                                            gtitle.Data());
