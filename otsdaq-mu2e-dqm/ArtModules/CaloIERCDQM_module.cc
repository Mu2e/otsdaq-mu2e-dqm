...
void ots::CaloIERCDQM::analyze(art::Event const& event) {
  ...
  if(!event.getByLabel(caloDigiTag_, caloDigisHandle)) {
    __COUT__ << "[CaloIERCDQM::" << __func__ << "] No calo digis found with tag " << caloDigiTag_.c_str() << std::endl;
    return;
  } else if(diagLevel_ > 0) {
    __COUT__ << "[CaloIERCDQM::" << __func__ << "] Calo digi collection handle found\n";
  }
  ...
  for (auto hist_singlechan_pair : histo_container->map_h1_dt_singlechan){
    __COUT__ << "[CaloIERCDQM::analyze] collecting single chan histogram "<< hist_singlechan_pair.second._Hist << std::endl;
    hists_to_send[moduleTag_+"/singleChan:replace"].push_back((TH1*)hist_singlechan_pair.second._Hist->Clone());
  }
  for (auto hist_singlechan_pairg : histo_container->map_g_dtevt_singlechan){
    __COUT__ << "[CaloIERCDQM::analyze] collecting single chan graph "<< hist_singlechan_pairg.second._Graph << std::endl;
    graphs_to_send[moduleTag_+"/singleChan"].push_back((TGraph*)hist_singlechan_pairg.second._Graph->Clone());
  }
  for (auto hist_sameboard_pair : histo_container->map_h1_dt_sameboard){
    __COUT__ << "[CaloIERCDQM::analyze] collecting same board histogram "<< hist_sameboard_pair.second._Hist << std::endl;
    hists_to_send[moduleTag_+"/sameBoard:replace"].push_back((TH1*)hist_sameboard_pair.second._Hist->Clone());
  }
  for (auto hist_sameboard_pairg : histo_container->map_g_dtevt_sameboard){
    __COUT__ << "[CaloIERCDQM::analyze] collecting same board graph "<< hist_sameboard_pairg.second._Graph << std::endl;
    graphs_to_send[moduleTag_+"/sameBoard"].push_back((TGraph*)hist_sameboard_pairg.second._Graph->Clone());
  }
  for (auto hist_diffboard_pair : histo_container->map_h1_dt_diffboard){
    __COUT__ << "[CaloIERCDQM::analyze] collecting diff board histogram "<< hist_diffboard_pair.second._Hist << std::endl;
    hists_to_send[moduleTag_+"/diffBoard:replace"].push_back((TH1*)hist_diffboard_pair.second._Hist->Clone());
  }
  for (auto hist_diffboard_pairg : histo_container->map_g_dtevt_diffboard){
    __COUT__ << "[CaloIERCDQM::analyze] collecting diff board graph "<< hist_diffboard_pairg.second._Graph << std::endl;
    graphs_to_send[moduleTag_+"/diffBoard"].push_back((TGraph*)hist_diffboard_pairg.second._Graph->Clone());
  }
