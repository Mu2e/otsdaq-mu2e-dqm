...
namespace ots {
  class CaloIERCDQMDebug : public art::EDAnalyzer {
  public:
    struct Config {
      ...
      fhicl::Atom<std::string>     caloDigiTag { Name("caloDigiTag"),      Comment("Module tag name") };
      ...
      fhicl::Atom<std::string>     spline      { Name("splineFilename" ) , Comment("spline root file path"), ""};
      fhicl::Atom<bool>            uset0       { Name("uset0"),            Comment("Use t0 instead of fitting with templates"), false};
      fhicl::Atom<int>             skipAfterN  { Name("skipAfterN"),       Comment("Don't fit after N hits"), -1};
};

    typedef art::EDAnalyzer::Table<Config> Parameters;

    explicit CaloIERCDQMDebug(Parameters const& conf);
...
ots::CaloIERCDQMDebug::CaloIERCDQMDebug(Parameters const& conf)
  : art::EDAnalyzer(conf), conf_(conf()), port_(conf().port()), address_(conf().address()),
    moduleTag_(conf().moduleTag()), caloDigiTag_(conf().caloDigiTag()),
    histType_(conf().histType()), 
    freqDQM_(conf().freqDQM()), diagLevel_(conf().diag()), evtCounter_(0), 
    doOnspillHist_(false), doOffspillHist_(false), splineFilename_(conf().spline()),
    uset0_(conf().uset0()), skipAfterN_(conf().skipAfterN()) {
  //histSender_  = new HistoSender(address_, port_);
  
  if (diagLevel_>0){
    __COUT__ << "[CaloIERCDQMDebug::analyze] DQM for "<< histType_[0] << std::endl;
  }
  ...
  f_spline = new TF1("f_spline",
             [this](double *x, double *par) {return par[0]*this->spline->Eval(x[0]-par[1])+par[2];}
             ,3.,25.,3);
...
void ots::CaloIERCDQMDebug::beginJob() {
  __COUT__ << "[CaloIERCDQMDebug::beginJob] Beginning job" << std::endl;
}

void ots::CaloIERCDQMDebug::analyze(art::Event const& event) {
  ++evtCounter_;
  ...
  if(!event.getByLabel(caloDigiTag_, caloDigisHandle)) {
    __COUT__ << "[CaloIERCDQMDebug::" << __func__ << "] No calo digis found with tag " << caloDigiTag_.c_str() << std::endl;
    return;
  } else if(diagLevel_ > 0) {
    __COUT__ << "[CaloIERCDQMDebug::" << __func__ << "] Calo digi collection handle found\n";
  }
  ...
  if(!caloDigis) {
    ...
    __COUT__ << "[CaloIERCDQMDebug::" << __func__ << "] Calo digi collection found\n";
  }

  if (!caloDigis){
    TLOG(TLVL_DEBUG) << "CaloDigi pointer is null";
    std::cout << "CaloDigi pointer is null\n";
  } else {
    TLOG(TLVL_DEBUG) << "CaloDigi pointer has size " << caloDigis->size();
    std::cout << "CaloDigi pointer has size " << caloDigis->size() << "\n";
  }
  ...
  for (auto hist_singlechan_pair : histo_container->map_h1_dt_singlechan){
    __COUT__ << "[CaloIERCDQMDebug::analyze] collecting single chan histogram "<< hist_singlechan_pair.second._Hist << std::endl;
    hists_to_send[moduleTag_+"_summary"].push_back((TH1*)hist_singlechan_pair.second._Hist->Clone());
  }
  for (auto hist_singlechan_pairg : histo_container->map_g_dtevt_singlechan){
    __COUT__ << "[CaloIERCDQMDebug::analyze] collecting single chan graph "<< hist_singlechan_pairg.second._Graph << std::endl;
    graphs_to_send[moduleTag_+"_summary"].push_back((TGraph*)hist_singlechan_pairg.second._Graph->Clone());
  }
  for (auto hist_sameboard_pair : histo_container->map_h1_dt_sameboard){
    __COUT__ << "[CaloIERCDQMDebug::analyze] collecting same board histogram "<< hist_sameboard_pair.second._Hist << std::endl;
    hists_to_send[moduleTag_+"_summary"].push_back((TH1*)hist_sameboard_pair.second._Hist->Clone());
  }
  for (auto hist_sameboard_pairg : histo_container->map_g_dtevt_sameboard){
    __COUT__ << "[CaloIERCDQMDebug::analyze] collecting same board graph "<< hist_sameboard_pairg.second._Graph << std::endl;
    graphs_to_send[moduleTag_+"_summary"].push_back((TGraph*)hist_sameboard_pairg.second._Graph->Clone());
  }
  for (auto hist_diffboard_pair : histo_container->map_h1_dt_diffboard){
    __COUT__ << "[CaloIERCDQMDebug::analyze] collecting diff board histogram "<< hist_diffboard_pair.second._Hist << std::endl;
    hists_to_send[moduleTag_+"_summary"].push_back((TH1*)hist_diffboard_pair.second._Hist->Clone());
  }
  for (auto hist_diffboard_pairg : histo_container->map_g_dtevt_diffboard){
    __COUT__ << "[CaloIERCDQMDebug::analyze] collecting diff board graph "<< hist_diffboard_pairg.second._Graph << std::endl;
    graphs_to_send[moduleTag_+"_summary"].push_back((TGraph*)hist_diffboard_pairg.second._Graph->Clone());
  }


  std::cout << "Sending histograms. First hist has "<<hists_to_send[moduleTag_+"_summary"].front()->GetEntries()<<" entries\n";
...
void ots::CaloIERCDQMDebug::summary_fill(CaloIERCDQMHistoContainer       *histos, 
                                    const mu2e::CaloDigiCollection  *caloDigis) {
  ...
  if (uset0_){ //Use t0 instead of fitting
    ...
    for (auto pair : digiMap){
      ...
      for (uint ihit=0; ihit<pair.second.size(); ihit++){
        ...
        if (fitStatus >= 0){
          timeMap[pair.first].push_back(5.*(caloDigis->at(idx).t0()+f_spline->GetParameter(1)));
  ...
  for (auto pair : timeMap){
    ...
    if (pair.second.size()>1){ //there must be at least 2 hits
      if (histo_container->map_h1_dt_singlechan.find(thisID) == histo_container->map_h1_dt_singlechan.end()){//This hist doesn't exist yet
        TString hname = Form("h1_dt_schan_%d",thisID);
        TString htitle = Form("[SAME CHANNEL] dt between hit 0 and hit 1 of sipm %d",thisID);
        histo_container->BookSingleChannel(tfs,thisID,hname.Data(),htitle.Data(),600,10009,10011);
        ...
        histo_container->BookSingleChannelG(tfs,thisID,gname.Data(),gtitle.Data());
      ...
      histo_container->map_g_dtevt_singlechan[thisID]._Graph->AddPoint(this_eventNumber,dt);
    }
  }
  ...
  for (auto pair : timeMap){
    ...
    for (auto nextpair : timeMap){
      ...
      if (thisDTCROC == nextDTCROC){ //We found a different channel in the same board
        ...
        if (histo_container->map_h1_dt_sameboard.find(idpair) == histo_container->map_h1_dt_sameboard.end()){//This hist doesn't exist yet
          ...
          histo_container->BookSameBoard(tfs,idpair,hname.Data(),htitle.Data(),5000,-50,50);
          ...
          histo_container->BookSameBoardG(tfs,idpair,gname.Data(),gtitle.Data());
        ...
        histo_container->map_g_dtevt_sameboard[idpair]._Graph->AddPoint(this_eventNumber,dt);
      }
    }
  }
    ...
    for (auto pair : timeMap){
      ...
      if (thisChan == chan){ //Found the channel we want to plot
        if (refID < 0){//It's the first one, don't do anything
          ...
          if (histo_container->map_h1_dt_diffboard.find(idpair) == histo_container->map_h1_dt_diffboard.end()){//This hist doesn't exist yet
            ...
            histo_container->BookDiffBoard(tfs,idpair,hname.Data(),htitle.Data(),20000,-200,200);
            ...
            histo_container->BookDiffBoardG(tfs,idpair,gname.Data(),gtitle.Data());
          ...
          histo_container->map_g_dtevt_diffboard[idpair]._Graph->AddPoint(this_eventNumber,dt);
        }
      }
    }
  }
...
void ots::CaloIERCDQMDebug::endJob() {}

void ots::CaloIERCDQMDebug::beginRun(const art::Run& run) {}

DEFINE_ART_MODULE(ots::CaloIERCDQMDebug)
