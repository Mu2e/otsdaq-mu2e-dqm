...
namespace mu2e
{
  ...
  private:
    ...
    {
      return disk * kChannelsPerDisk + encodeChannel(disk, boardID, chanID);  // 0 to 3199
    }
    ...
    // Helpers
    // -----------------------
    bool modeEnabled(MapMode m) const
    {
      return enabledModes_[modeIndex(m)];
    }

    bool streamModeEnabled(MapMode m) const
    {
      return streamEnabledModes_[modeIndex(m)];
    }
    ...
    static constexpr size_t modeIndex(MapMode m)
    {
    ...
    void ensureLiveWaveformBooked(int disk, int boardID, int chanID, int rawId, int sipmId);
  ...
  {
    const int cidx = channelIndex(disk, boardID, chanID);
    if(cidx < 0 || cidx >= kTotalChannels) return;
    if(h_rms_dist_[(size_t)cidx]) return;

    const int bidx = boardIndex(disk, boardID);
    if(bidx < 0 || bidx >= kTotalBoards) return;
    if(!boardRmsDir_[(size_t)bidx]) return;

    art::TFileDirectory& dir = *boardRmsDir_[(size_t)bidx];

    h_rms_dist_[(size_t)cidx] =
      dir.make<TH1F>(
                     Form("D%d_B%03d_C%02d_RMSDist", disk, boardID, chanID),
  ...
  void CaloDigiDQM::ensureMaxDistBooked(int disk, int boardID, int chanID)
  {
    const int cidx = channelIndex(disk, boardID, chanID);
    if(cidx < 0 || cidx >= kTotalChannels) return;
    if(h_max_dist_[(size_t)cidx]) return;

    const int bidx = boardIndex(disk, boardID);
    if(bidx < 0 || bidx >= kTotalBoards) return;
    if(!boardMaxDir_[(size_t)bidx]) return;

    art::TFileDirectory& dir = *boardMaxDir_[(size_t)bidx];

    h_max_dist_[(size_t)cidx] =
      dir.make<TH1F>(
  ...
  void CaloDigiDQM::ensureAsymDistBooked(int disk, int boardID, int chanID)
  {
    const int cidx = channelIndex(disk, boardID, chanID);
    if(cidx < 0 || cidx >= kTotalChannels) return;
    if(h_asym_dist_[(size_t)cidx]) return;

    const int bidx = boardIndex(disk, boardID);
    if(bidx < 0 || bidx >= kTotalBoards) return;
    if(!boardAsymDir_[(size_t)bidx]) return;

    art::TFileDirectory& dir = *boardAsymDir_[(size_t)bidx];

    h_asym_dist_[(size_t)cidx] =
      dir.make<TH1F>(
    ...
    if(!boardHistDir_[(size_t)bidx] || !boardChanDir_[(size_t)bidx])
      {
        ...
        boardBaselineDir_[(size_t)bidx] =
          std::make_unique<art::TFileDirectory>(boardChanDir_[(size_t)bidx]->mkdir("Baseline"));

        boardRmsDir_[(size_t)bidx] =
          std::make_unique<art::TFileDirectory>(boardChanDir_[(size_t)bidx]->mkdir("RMS"));

        boardMaxDir_[(size_t)bidx] =
          std::make_unique<art::TFileDirectory>(boardChanDir_[(size_t)bidx]->mkdir("Max"));

        boardAsymDir_[(size_t)bidx] =
          std::make_unique<art::TFileDirectory>(boardChanDir_[(size_t)bidx]->mkdir("Asym"));
      }
    ...
    , enableDiskMaps_(config().enableDiskMaps())
  {
    ...
    h_waveform_size_  =
      globalDir_->make<TH1F>("h_waveform_size",
    ...
    h_occupancy_sparse_ =
      globalDir_->make<TH1F>("h_occ_sparse",
                             ...
                             axisSparse.nBins, axisSparse.xMin, axisSparse.xMax);
    ...
    h_baseline_dense_ =
      makeGlobalProfile("h_base_dense",
    ...
    h_rms_sparse_ =
      makeGlobalProfile("h_rms_sparse",
    ...
    h_amp_sparse_ =
      makeGlobalProfile("h_amp_sparse",
    ...
    h_maxval_sparse_ =
      makeGlobalProfile("h_max_sparse",
  ...
  void CaloDigiDQM::analyze(art::Event const& event)
  {
    ...
    for(const auto& digi : caloDigis)
      {
        ...
        const float rms =
          (mean_sq > baseline * baseline) ? std::sqrt(mean_sq - baseline * baseline) : 0.0f;
        ...
        ensureFirstHitBooked(disk, boardID, chanID, rawId, sipmId, waveform, wfSize, baseline);
        ...
        markFeat(sipmId, SipmFeat{amp, baseline, rms, (double)ampRaw, disk, boardID, chanID, cidx});
    ...
    const bool doDiskMaps    = enableDiskMaps_ && (diskMapPeriod > 0) && (eventCounter_ % diskMapPeriod == 0);
            ...
            for(int blocal = 0; blocal < kBoardsPerDisk; ++blocal)
              {
                ...
                if(bh.occ)  hists_to_send[groupPath].push_back(bh.occ);
                if(bh.base) hists_to_send[groupPath].push_back(bh.base);
                if(bh.rms)  hists_to_send[groupPath].push_back(bh.rms);
                if(bh.max)  hists_to_send[groupPath].push_back(bh.max);
              }
          }
      }
        ...
        for(auto m : streamModes_)
          {
            ...
            if(disk0Maps_[midx]) hists_to_send[groupPath].push_back(disk0Maps_[midx]);
            if(disk1Maps_[midx]) hists_to_send[groupPath].push_back(disk1Maps_[midx]);
          }
      }
            ...
            for(int blocal = 0; blocal < kBoardsPerDisk; ++blocal)
              {
                ...
                std::string livePath =
                  Form("%s/Waveforms/Disk%d/Board%03d:replace", moduleTag_.c_str(), disk, boardID);
                std::string oneHitPath =
                  Form("%s/OneHitWaveforms/Disk%d/Board%03d:replace", moduleTag_.c_str(), disk, boardID);

                for(int chan = 0; chan < kChannelsPerBoard; ++chan)
                  {
                    const int cidx = disk * kChannelsPerDisk + blocal * kChannelsPerBoard + chan;

                    if(liveWf_[(size_t)cidx])   hists_to_send[livePath].push_back(liveWf_[(size_t)cidx]);
                    if(oneHitWf_[(size_t)cidx]) hists_to_send[oneHitPath].push_back(oneHitWf_[(size_t)cidx]);
                  }
              }
          }
      }
  ...
  void CaloDigiDQM::endJob()
  {
    ...
    struct Row { int cidx; WaveformSizeStats st; };
    ...
    for(int cidx = 0; cidx < kTotalChannels; ++cidx)
      {
        if(!chSeen_[(size_t)cidx]) continue;
        ++seenCount;
