...
namespace mu2e
{
  ...
  private:
    ...
    static constexpr const char* kReferenceFile =
      "reference.root";
    // -----------------------
  ...
  if(doSummariesEvent)
    {
      ...
      if(h_occupancy_sparse_) g.push_back(h_occupancy_sparse_);
      if(ref_h_occ_dense_)    g.push_back(ref_h_occ_dense_);
      if(h_occupancy_dense_)  g.push_back(h_occupancy_dense_);

      if(h_baseline_sparse_)  g.push_back(h_baseline_sparse_);
      if(ref_h_base_dense_)   g.push_back(ref_h_base_dense_);
      if(h_baseline_dense_)   g.push_back(h_baseline_dense_);

      if(h_rms_sparse_)       g.push_back(h_rms_sparse_);
      if(ref_h_rms_dense_)    g.push_back(ref_h_rms_dense_);
      if(h_rms_dense_)        g.push_back(h_rms_dense_);

      if(h_maxval_sparse_)    g.push_back(h_maxval_sparse_);
      if(ref_h_max_dense_)    g.push_back(ref_h_max_dense_);
      if(h_maxval_dense_)     g.push_back(h_maxval_dense_);

      if(ref_h_asym_)         g.push_back(ref_h_asym_);
      if(h_asymmetry)         g.push_back(h_asymmetry);

      if(h_global_board_dist_)        g.push_back(h_global_board_dist_);
      if(h_global_board_vs_channel_)  g.push_back(h_global_board_vs_channel_);
      if(h_global_waveform_density_)  g.push_back(h_global_waveform_density_);
      if(h_waveform_size_)            g.push_back(h_waveform_size_);
      if(h_skip_reason_)              g.push_back(h_skip_reason_);
          ...
          for(int blocal = 0; blocal < kBoardsPerDisk; ++blocal)
            {
              ...
              if(disk == 0 && boardID == 27)
                {
                  if(ref_D0_B027_Occupancy_) hists_to_send[groupPath].push_back(ref_D0_B027_Occupancy_);
                  if(ref_D0_B027_Baseline_)  hists_to_send[groupPath].push_back(ref_D0_B027_Baseline_);
                  if(ref_D0_B027_RMS_)       hists_to_send[groupPath].push_back(ref_D0_B027_RMS_);
                  if(ref_D0_B027_Max_)       hists_to_send[groupPath].push_back(ref_D0_B027_Max_);
                }
