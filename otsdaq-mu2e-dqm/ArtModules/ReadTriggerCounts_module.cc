...
ReadTriggerCounts::ReadTriggerCounts(fhicl::ParameterSet const &pset)
    : art::EDAnalyzer(pset),
      ...
      _hsCprTag(pset.get<art::InputTag>(
          "cprHelixSeedCollection",
          "CalHelixFinderDe:Positive")), // , "KFFDeMHPar")),
      _hsTprTag(pset.get<art::InputTag>(
          "tprHelixSeedCollection",
          "HelixFinderDe:Positive")), // , "KFFDeMHPar")),
      _ksTag(pset.get<art::InputTag>("kalSeedCollection", "KFFDeMHPar")),
      _trkQualTag(
          pset.get<art::InputTag>("trackQualCollection", "TrkQualDeMHPar")),
      _vdTag(pset.get<art::InputTag>(
          "vdStepPoints", "NOTNOW")), // , "compressDigiMCs:virtualdetector")),
      ...
      _nProcess(pset.get<float>("nEventsProcessed", 1.)), _minPOT(1e6),
      _maxPOT(4e8) {
...
void ReadTriggerCounts::analyze(const art::Event &event) {
  ...
  metricMan->sendMetric("TriggerCounts.TotalEvents", 1, "events", 2,
                        artdaq::MetricMode::Accumulate);
  metricMan->sendMetric("TriggerCounts.TotalEventsCumulative", 1, "events", 2,
                        artdaq::MetricMode::LastPoint);
  for (unsigned int i = 0; i < trigNavig.getTrigPaths().size(); ++i) {
    //      if (trigResults->accept(i)){
    std::string path = trigNavig.getTrigPathName(
        i); // incorporates the reconstruction algorithm
    if (trigNavig.accepted(path)) {
      triggerStreamCounts["TriggerCounts." + path]++;
      metricMan->sendMetric("TriggerCounts." + path, 1, "events", 2,
                            artdaq::MetricMode::Accumulate);
      metricMan->sendMetric("TriggerCounts." + path + "Cumulative", 1, "events",
                            2, artdaq::MetricMode::LastPoint);
      ...
      if (hasNotPassedTrigger) {
        ...
        metricMan->sendMetric("TriggerCounts.TotalPassedTrigger", 1, "events",
                              2, artdaq::MetricMode::Accumulate);
        metricMan->sendMetric("TriggerCounts.TotalPassedTriggerCumulative", 1,
                              "events", 2, artdaq::MetricMode::LastPoint);
