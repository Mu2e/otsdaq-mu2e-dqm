...
ots::IntegrationDQM::IntegrationDQM(Parameters const& conf)
  : art::EDAnalyzer(conf), conf_(conf()), port_(conf().port()), address_(conf().address()), sendHists_(conf().sendHists()),
    ...
    calo_tick_time_(5.f), trk_tick_time_(1.f/*FIXME*/), crv_tick_time_(12.5f), stm_tick_time_(1.f/*FIXME*/) {
...
// void ots::IntegrationDQM::FormatHists(SimpleDQMHistoContainer& hists) {  //MM: Moved to IntegrationHistoContainer::BookHist

void ots::IntegrationDQM::beginJob() {
  __COUT__ << "[IntegrationDQM::beginJob] Beginning job" << std::endl;

  // Calorimeter plots
  if(caloDigiTag_ != "") hists_._calo_hists.init(tfs_, "IntegrationDQM", nCaloSiPMs_);

  //Tracker plots
  if(trkDigiTag_ != "") hists_._trk_hists.init(tfs_, "IntegrationDQM", 0);

  //CRV plots
  if(crvDigiTag_ != "") hists_._crv_hists.init(tfs_, "IntegrationDQM", 0);

  //STM plots
  if(stmDigiTag_ != "") hists_._stm_hists.init(tfs_, "IntegrationDQM");
...
void ots::IntegrationDQM::analyze(art::Event const& event) {
  ...
  if(fillCalo) summary_calo_fill(event, fill_pulses);
  if(fillTrk ) summary_trk_fill (event, fill_pulses);
  if(fillCRV ) summary_crv_fill (event, fill_pulses);
  if(fillSTM ) summary_stm_fill (event, fill_pulses);
  summary_global_fill(event);

  // Decide whether or not to send the current histograms
  if(!send_summary || !sendHists_) return;

  //send a packet, replace histogram on other end so no need to reset the histograms
  std::map<std::string,std::vector<TH1*>>   hists_to_send;

  // Send calorimeter histograms
  if(fillCalo) hists_to_send["integration_summary/calo:replace"] = hists_._calo_hists.hists_to_send();

  // Send tracker histograms
  if(fillTrk) hists_to_send["integration_summary/trk:replace"] = hists_._trk_hists.hists_to_send();

  // Send CRV histograms
  if(fillCRV) hists_to_send["integration_summary/crv:replace"] = hists_._crv_hists.hists_to_send();

  // Send STM histograms
  if(fillSTM) hists_to_send["integration_summary/stm:replace"] = hists_._stm_hists.hists_to_send();

  // send global histograms
  hists_to_send["integration_summary/global:replace"] = hists_._global_hists.hists_to_send();

  // append run number to all histograms
  for(auto& entry : hists_to_send) {
    for(auto hist : entry.second) hist->SetTitle(Form("%s: Run %i", hist->GetTitle(), event.run()));
  }
...
void ots::IntegrationDQM::retrieve_data(art::Event const& event) {
  ...
  if(caloDigiTag_ != "") {
    ...
    if(!event.getByLabel(caloDigiTag_, digisH)) {
      __COUT__ << "[IntegrationDQM::" << __func__ << "] No calo digis found with tag " << caloDigiTag_.c_str() << std::endl;
    } else {
  ...
  if(trkDigiTag_ != "") {
    ...
    if(!event.getByLabel(trkDigiTag_, trkDigisH)) {
      __COUT__ << "[IntegrationDQM::" << __func__ << "] No Trk digis found with tag " << trkDigiTag_.c_str() << std::endl;
    } else {
  ...
  if(crvDigiTag_ != "") {
	...
	__COUT__ << "[IntegrationDQM::" << __func__ << "] No CRV decoders found with tag " << crvDigiTag_.c_str() << std::endl;
      } else {
	...
	__COUT__ << "[IntegrationDQM::" << __func__ << "] No CRV digis found with tag " << crvDigiTag_.c_str() << std::endl;
      } else {
  ...
  if(stmDigiTag_ != "") {
    ...
    if(!event.getByLabel(stmDigiTag_, digisH)) {
      __COUT__ << "[IntegrationDQM::" << __func__ << "] No stm digis found with tag " << stmDigiTag_.c_str() << std::endl;
    } else {
...
void ots::IntegrationDQM::summary_calo_fill(art::Event const& event, const bool fill_pulses) {
  ...
  if(diagLevel_ > 0) std::cout << "[IntegrationDQM::" << __func__ << "] Calo data: " << digis->size() << " digis\n";
  for(auto& digi : *digis) {
    ...
    if(diagLevel_ > 0) std::cout << "--> Calo digi: "
				 << " t0 = " << t0 << " t(fit) = " << time << " (diff = " << time - t0 << ") peak = " << peak
    ...
    hists.sipms._Hist->Fill(Form("D%02iR%iC%02i", dtc, roc, channel), 1.); //Fill the SiPM ID as the x-axis labels
    if(new_sipm) { //FIXME: Sorting the labels here doesn't appear to work, perhaps needs to be done when drawing
      try {
	hists.sipms._Hist->GetXaxis()->LabelsOption("a"); //re-sort the bin labels if a new one is added
      } catch(...){}
    }
    ...
    if(fill_pulses) {
      if(new_sipm) sipmIDs_[sipmID] = sipmIDs_.size();
      const size_t index = sipmIDs_[sipmID];
      if(index >= hists.pulses.size()) continue;
...
void ots::IntegrationDQM::summary_trk_fill(art::Event const& event, const bool fill_pulses) {
  ...
  hists.digi_count._Hist->Fill(digis->size());
  for(auto& digi : *digis) {
    hists.digi_t0._Hist->Fill(digi.TDC()[0]*trk_tick_time_); //FIXME: Convert to ns
  }

}

void ots::IntegrationDQM::summary_crv_fill(art::Event const& event, const bool fill_pulses) {
  ...
  if(using_decoders) {
    ...
    if(diagLevel_ > 0) std::cout << "[IntegrationDQM::" << __func__ << "] CRV data: " << nSubEvents << " sub-events, " << gr_infos.size() << " GR Infos\n";
    for(const auto& gr_info : gr_infos) {
      if(diagLevel_ > 0) std::cout << "--> CRV data entry: EWT = " << gr_info.EWTCount
      ...
      hists.injection_time._Hist->Fill(gr_info.injectionTime * crv_tick_time_); // ns
...
//STM
void ots::IntegrationDQM::summary_stm_fill(art::Event const& event, const bool fill_pulses) {
...
void ots::IntegrationDQM::summary_global_fill(art::Event const& event) {
  ...
  if(max_bin > 0.) {
    ...
    if(rel_diff > 0.01)    IntegrationDQMHistoContainer::BadHist    (hdigi_count);
    else if(rel_diff > 0.) IntegrationDQMHistoContainer::WarningHist(hdigi_count);
    else                   IntegrationDQMHistoContainer::GoodHist   (hdigi_count);
  }
  ...
  if(fillTrk  && fillCalo) hists.trk_calo_deltat0._Hist->Fill(trk_t0  - calo_t0);
  if(fillTrk  && fillCRV ) hists.trk_crv_deltat0 ._Hist->Fill(trk_t0  - crv_t0 );
  if(fillTrk  && fillSTM ) hists.trk_stm_deltat0 ._Hist->Fill(trk_t0  - stm_t0 );
  if(fillCalo && fillCRV ) hists.calo_crv_deltat0._Hist->Fill(calo_t0 - crv_t0 );
  if(fillCalo && fillSTM ) hists.calo_stm_deltat0._Hist->Fill(calo_t0 - stm_t0 );
  if(fillCRV  && fillSTM ) hists.crv_stm_deltat0 ._Hist->Fill(crv_t0  - stm_t0 );

}

std::vector<mu2e::CRVDataDecoder::CRVGlobalRunInfo> ots::IntegrationDQM::decode_crv(const std::vector<mu2e::CRVDataDecoder>* decoders) {
  if(!decoders) return {};
  ...
  for(size_t iSubEvent = 0; iSubEvent < nSubEvents; ++iSubEvent) {
    ...
    for(size_t iDataBlock = 0; iDataBlock < CRVDataDecoder.block_count(); ++iDataBlock) {
	  ...
	  if(crvRocHeader==nullptr) continue;
