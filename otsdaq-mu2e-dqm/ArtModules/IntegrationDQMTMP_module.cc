...
namespace ots {
  class IntegrationDQMTMP : public art::EDAnalyzer {
  public:
    struct Config {
    ...
    explicit IntegrationDQMTMP(Parameters const& conf);
...
ots::IntegrationDQMTMP::IntegrationDQMTMP(Parameters const& conf)
...
// void ots::IntegrationDQMTMP::FormatHists(SimpleDQMHistoContainer& hists) { 
//   for (auto &hist : hists.histograms) { 
//     hist._Hist->SetLineColor(kAzure+2);
//     hist._Hist->SetFillColor(kAzure-9);
//     hist._Hist->SetLineWidth(2);
//     hist._Hist->GetXaxis()->SetTitleSize(.04);
//     hist._Hist->GetYaxis()->SetTitleSize(.04);
//     // Whatever else
//   }
// }

void ots::IntegrationDQMTMP::beginJob() {
  ...
  // // Formatting
  // FormatHists(calo_hists_);
  // FormatHists(trk_hists_);
  // FormatHists(crv_hists_);
  // FormatHists(stm_hists_);
  // FormatHists(global_hists_);

}

void ots::IntegrationDQMTMP::analyze(art::Event const& event) {
  ++evtCounter_;
...
void ots::IntegrationDQMTMP::retrieve_data(art::Event const& event) {
  // Initialize to null
  calo_digis_   = nullptr;
  trk_digis_    = nullptr;
  crv_digis_    = nullptr;
...
void ots::IntegrationDQMTMP::summary_calo_fill(art::Event const& event, const bool fill_pulses) {
  ...
  for(auto& digi : *digis) {
    ...
    hists.sipms._Hist->Fill(Form("%i", sipmID), 1.); //Fill the SiPM ID as the x-axis labels, won't be ordered...
    ...
    if(fill_pulses) {
      if(!sipmIDs_.count(sipmID)) sipmIDs_[sipmID] = sipmIDs_.size();
...
void ots::IntegrationDQMTMP::summary_trk_fill(art::Event const& event, const bool fill_pulses) {
  ...
  hists.digi_count._Hist->Fill(digis->size());
  for(auto& digi : *digis) {
    hists.digi_t0._Hist->Fill(digi.TDC()[0]*1.f); //FIXME: Convert to ns
  }

}

void ots::IntegrationDQMTMP::summary_crv_fill(art::Event const& event, const bool fill_pulses) {
  ...
  for(const auto& gr_info : gr_infos) {   
    ...
    hists.injection_time._Hist->Fill(gr_info.injectionTime * 12.5f); // ns
...
//STM
void ots::IntegrationDQMTMP::summary_stm_fill(art::Event const& event, const bool fill_pulses) {
  ...
  for(auto& digi : *digis) {
    ...
    hists.digi_t0._Hist->Fill(digi.trigTimeOffset()*1.f); //FIXME
  }
...
void ots::IntegrationDQMTMP::summary_global_fill(art::Event const& event) {
...
std::vector<mu2e::CRVDataDecoder::CRVGlobalRunInfo> ots::IntegrationDQMTMP::decode_crv(const std::vector<mu2e::CRVDataDecoder>* decoders) {
  if(!decoders) return {};
...
void ots::IntegrationDQMTMP::endJob() {}

void ots::IntegrationDQMTMP::beginRun(const art::Run& run) {}

DEFINE_ART_MODULE(ots::IntegrationDQMTMP)
