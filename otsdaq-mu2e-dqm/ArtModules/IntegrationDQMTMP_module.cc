...
ots::IntegrationDQMTMP::IntegrationDQMTMP(Parameters const &conf)
    : art::EDAnalyzer(conf), conf_(conf()), port_(conf().port()),
      ...
      evtCounter_(0), calo_pulse_fitter_(conf().caloFitter().c_str()),
...
void ots::IntegrationDQMTMP::summary_calo_fill(art::Event const &event,
  ...
  for (auto &digi : *digis) {
    ...
    hists.sipms._Hist->Fill(
        Form("%i", sipmID),
        1.); // Fill the SiPM ID as the x-axis labels, won't be ordered...
...
void ots::IntegrationDQMTMP::summary_trk_fill(art::Event const &event,
                                              const bool fill_pulses) {
...
void ots::IntegrationDQMTMP::summary_crv_fill(art::Event const &event,
                                              const bool fill_pulses) {
...
// STM
void ots::IntegrationDQMTMP::summary_stm_fill(art::Event const &event,
                                              const bool fill_pulses) {
...
std::vector<mu2e::CRVDataDecoder::CRVGlobalRunInfo>
ots::IntegrationDQMTMP::decode_crv(
