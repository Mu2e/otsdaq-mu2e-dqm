...
public:
  struct Config {
    ...
    fhicl::Atom<bool> runOffline{
        Name("runOffline"), Comment("Run without sending histograms"), false};
  };
  ...
  void summary_calo_fill(art::Event const &event,
                         SimpleDQMHistoContainer &hists,
                         const bool fill_pulses);
  void summary_trk_fill(art::Event const &event, SimpleDQMHistoContainer &hists,
                        const bool fill_pulses);
  void summary_crv_fill(art::Event const &event, SimpleDQMHistoContainer &hists,
                        const bool fill_pulses);
  void summary_stm_fill(art::Event const &event, SimpleDQMHistoContainer &hists,
                        const bool fill_pulses);
...
private:
  ...
  SimpleDQMHistoContainer calo_hists_, trk_hists_, crv_hists_, stm_hists_,
      global_hists_;
...
ots::IntegrationDQMOffline::IntegrationDQMOffline(Parameters const &conf)
    : art::EDAnalyzer(conf), conf_(conf()), port_(conf().port()),
      address_(conf().address()), caloDigiTag_(conf().caloDigiTag()),
      trkDigiTag_(conf().trkDigiTag()), crvDigiTag_(conf().crvDigiTag()),
      stmDigiTag_(conf().stmDigiTag()), freqDQM_(conf().freqDQM()),
      freqCaloPulse_(conf().freqCaloPulse()), nCaloSiPMs_(conf().nCaloSiPMs()),
      diagLevel_(conf().diag()), evtCounter_(0),
      calo_pulse_fitter_(conf().caloFitter().c_str()),
      histSender_(conf().runOffline() ? nullptr
                                      : new HistoSender(address_, port_)),
...
void ots::IntegrationDQMOffline::beginJob() {
  ...
  if (caloDigiTag_ != "") {
    ...
    calo_hists_.BookSummaryHistos(tfs_, "calo_digi_count",
                                  "Calo digi count;N(digis);Counts", 50, 0.f,
                                  50.f, "IntegrationDQMOffline");
    calo_hists_.BookSummaryHistos(tfs_, "calo_digi_t0",
                                  "Calo digi t_{0};digi t_{0};Counts", 100, 0.f,
                                  50.e3f, "IntegrationDQMOffline");
    calo_hists_.BookSummaryHistos(
        tfs_, "calo_digi_dt0", "Calo digi #Deltat_{0};digi #Deltat_{0};Counts",
        101, -252.5f, 252.5f, "IntegrationDQMOffline");
    calo_hists_.BookSummaryHistos(tfs_, "calo_digi_tfit",
                                  "Calo digi t_{fit};digi t_{fit};Counts", 100,
                                  0.f, 50.e3f, "IntegrationDQMOffline");
    calo_hists_.BookSummaryHistos(
        tfs_, "calo_digi_dtfit",
        "Calo digi #Deltat_{fit};digi #Deltat_{fit};Counts", 800, -100.f, 100.f,
        "IntegrationDQMOffline");
    calo_hists_.BookSummaryHistos(
        tfs_, "calo_digi_tfit_t0",
        "Calo #Deltat_{fit};digi t_{fit} - t_{0};Counts", 500, -10.f, 10.f,
        "IntegrationDQMOffline");
    nCaloSummary_ = calo_hists_.histograms.size();

    // Book calo pulse histograms
    for (int ihist = 0; ihist < nCaloSiPMs_; ++ihist) {
      calo_hists_.BookSummaryHistos(tfs_, Form("calo_digi_wave_%i", ihist),
                                    "Calo SiPM waveform;bin;Counts", 30, 0.f,
                                    30.f, "IntegrationDQMOffline", "replace");
    }
  }
  ...
  if (trkDigiTag_ != "") {
    ...
    trk_hists_.BookSummaryHistos(tfs_, "trk_digi_count",
                                 "Tracker digi count;N(digis);Counts", 500, 0.f,
                                 500.f, "IntegrationDQMOffline");
    trk_hists_.BookSummaryHistos(tfs_, "trk_digi_t0",
                                 "Tracker digi t_{0};digi t_{0};Counts", 100,
                                 0.f, 50.e3f, "IntegrationDQMOffline");
  ...
  if (crvDigiTag_ != "") {
    ...
    crv_hists_.BookSummaryHistos(tfs_, "crv_digi_count",
                                 "CRV digi count;N(digis);Counts", 30, 0.f,
                                 30.f, "IntegrationDQMOffline");
    crv_hists_.BookSummaryHistos(tfs_, "crv_digi_t0",
                                 "CRV digi t_{0};digi t_{0};Counts", 100, 0.f,
                                 50.e3f, "IntegrationDQMOffline");

    // Book CRV timing histograms
    crv_hists_.BookSummaryHistos(tfs_, "crv_ewt_dist",
                                 "EWT Distribution;EWT;Entries", 1021, -10.5f,
                                 1010.5f, "IntegrationDQMOffline");
    crv_hists_.BookSummaryHistos(
        tfs_, "crv_injection_time",
        "Injection Time Distribution;#Deltat_{0};Entries", 100, 0.f, 1000.f,
        "IntegrationDQMOffline");
    crv_hists_.BookSummaryHistos(tfs_, "crv_injection_window",
                                 "Injection Window Distribution;t_{0};Entries",
                                 100, 0.f, 1000.f, "IntegrationDQMOffline");
    crv_hists_.BookSummaryHistos(tfs_, "crv_marker_count",
                                 "Marker Counter;EWT;Marker Counter", 1021,
                                 -10.5f, 1010.5f, "IntegrationDQMOffline");
    crv_hists_.BookSummaryHistos(tfs_, "crv_ewt_count",
                                 "EWT Counter;EWT;EWT Counter", 1021, -10.5f,
                                 1010.5f, "IntegrationDQMOffline");
  ...
  if (stmDigiTag_ != "") {
    ...
    stm_hists_.BookSummaryHistos(tfs_, "stm_digi_count",
                                 "STM digi count;N(digis);Counts", 30, 0.f,
                                 30.f, "IntegrationDQMOffline");
    stm_hists_.BookSummaryHistos(tfs_, "stm_digi_t0",
                                 "STM digi DetID;digi DetID;Counts", 2, 0.f,
                                 2.f, "IntegrationDQMOffline");
    nSTMSummary_ = stm_hists_.histograms.size();

    // Book stm pulse histograms
    stm_hists_.BookSummaryHistos(tfs_, "stm_digi_wave",
                                 "STM SiPM waveform;waveform;Counts", 50, 0.f,
                                 50.f, "IntegrationDQMOffline", "replace");
  }

  // Global plots, always initialize to keep the indices aligned
  global_hists_.BookSummaryHistos(tfs_, "trk_calo_deltat0",
                                  "Track - Calo t_{0};#Deltat_{0};Counts", 500,
                                  -500.f, 500.f, "IntegrationDQMOffline");
  global_hists_.BookSummaryHistos(tfs_, "trk_crv_deltat0",
                                  "Track - CRV t_{0};#Deltat_{0};Counts", 500,
                                  -500.f, 500.f, "IntegrationDQMOffline");
  global_hists_.BookSummaryHistos(tfs_, "trk_stm_deltat0",
                                  "Track - STM t_{0};#Deltat_{0};Counts", 500,
                                  -500.f, 500.f, "IntegrationDQMOffline");
  global_hists_.BookSummaryHistos(tfs_, "calo_crv_deltat0",
                                  "Calo - CRV t_{0};#Deltat_{0};Counts", 500,
                                  -500.f, 500.f, "IntegrationDQMOffline");
  global_hists_.BookSummaryHistos(tfs_, "calo_stm_deltat0",
                                  "Calo - STM t_{0};#Deltat_{0};Counts", 500,
                                  -500.f, 500.f, "IntegrationDQMOffline");
  global_hists_.BookSummaryHistos(tfs_, "crv_stm_deltat0",
                                  "CRV - STM t_{0};#Deltat_{0};Counts", 500,
                                  -500.f, 500.f, "IntegrationDQMOffline");

  global_hists_.BookSummaryHistos(
      tfs_, "detector_digis",
      "Subdetector events above digi threshold;;N(events)", 5, 0, 5,
      "IntegrationDQMOffline");
}
...
void ots::IntegrationDQMOffline::analyze(art::Event const &event) {
  ...
  if (fillCalo)
    summary_calo_fill(event, calo_hists_, fill_pulses);
  if (fillTrk)
    summary_trk_fill(event, trk_hists_, fill_pulses);
  if (fillCRV)
    summary_crv_fill(event, crv_hists_, fill_pulses);
  if (fillSTM)
    summary_stm_fill(event, stm_hists_, fill_pulses);
  ...
  if (!conf_.runOffline()) {
    ...
    if (fillCalo) {
      ...
      for (size_t i = 0; i < nCaloSummary_; i++) {
        __COUT__
            << "[IntegrationDQMOffline::analyze] collecting summary histogram "
            << calo_hists_.histograms[i]._Hist << std::endl;
        hists_to_send["integration_summary/calo"].push_back(
            (TH1 *)calo_hists_.histograms[i]._Hist->Clone());
      ...
      if (fill_pulses) {
        for (size_t i = nCaloSummary_; i < calo_hists_.histograms.size(); i++) {
          __COUT__ << "[IntegrationDQMOffline::analyze] collecting summary "
                      "histogram "
                   << calo_hists_.histograms[i]._Hist << std::endl;
          hists_to_send["integration_summary/calo:replace"].push_back(
              (TH1 *)calo_hists_.histograms[i]._Hist->Clone());
          calo_hists_.histograms[i]._Hist->Reset();
        }
      }
    ...
    if (fillTrk) {
      ...
      for (size_t i = 0; i < nTrkSummary_; i++) {
        __COUT__
            << "[IntegrationDQMOffline::analyze] collecting summary histogram "
            << trk_hists_.histograms[i]._Hist << std::endl;
        hists_to_send["integration_summary/trk"].push_back(
            (TH1 *)trk_hists_.histograms[i]._Hist->Clone());
      ...
      if (fill_pulses) {
        for (size_t i = nTrkSummary_; i < trk_hists_.histograms.size(); i++) {
          __COUT__ << "[IntegrationDQMOffline::analyze] collecting summary "
                      "histogram "
                   << trk_hists_.histograms[i]._Hist << std::endl;
          hists_to_send["integration_summary/trk:replace"].push_back(
              (TH1 *)trk_hists_.histograms[i]._Hist->Clone());
          trk_hists_.histograms[i]._Hist->Reset();
        }
      }
    ...
    if (fillCRV) {
      ...
      for (size_t i = 0; i < nCRVSummary_; i++) {
        __COUT__
            << "[IntegrationDQMOffline::analyze] collecting summary histogram "
            << crv_hists_.histograms[i]._Hist << std::endl;
        hists_to_send["integration_summary/crv"].push_back(
            (TH1 *)crv_hists_.histograms[i]._Hist->Clone());
      ...
      if (fill_pulses) {
        for (size_t i = nCRVSummary_; i < crv_hists_.histograms.size(); i++) {
          __COUT__ << "[IntegrationDQMOffline::analyze] collecting summary "
                      "histogram "
                   << crv_hists_.histograms[i]._Hist << std::endl;
          hists_to_send["integration_summary/crv:replace"].push_back(
              (TH1 *)crv_hists_.histograms[i]._Hist->Clone());
          crv_hists_.histograms[i]._Hist->Reset();
        }
      }
    ...
    if (fillSTM) {
      ...
      for (size_t i = 0; i < nSTMSummary_; i++) {
        __COUT__
            << "[IntegrationDQMOffline::analyze] collecting summary histogram "
            << stm_hists_.histograms[i]._Hist << std::endl;
        hists_to_send["integration_summary/stm"].push_back(
            (TH1 *)stm_hists_.histograms[i]._Hist->Clone());
        stm_hists_.histograms[i]._Hist->Reset();
      }

      // send stm waveforms
      if (fill_pulses) {
        if (stm_hists_.histograms[nSTMSummary_]._Hist->Integral() >
            0.) { // only refresh if there's a waveform to send
          __COUT__ << "[IntegrationDQMOffline::analyze] collecting summary "
                      "histogram "
                   << stm_hists_.histograms[nSTMSummary_]._Hist << std::endl;
          hists_to_send["integration_summary/stm:replace"].push_back(
              (TH1 *)stm_hists_.histograms[nSTMSummary_]._Hist->Clone());
    ...
    for (size_t i = 0; i < global_hists_.histograms.size(); i++) {
      __COUT__
          << "[IntegrationDQMOffline::analyze] collecting summary histogram "
          << global_hists_.histograms[i]._Hist << std::endl;
      hists_to_send["integration_summary/global"].push_back(
          (TH1 *)global_hists_.histograms[i]._Hist->Clone());
...
void ots::IntegrationDQMOffline::retrieve_data(art::Event const &event) {
  ...
  if (caloDigiTag_ != "") {
    ...
    if (!event.getByLabel(caloDigiTag_, digisH)) {
      __COUT__ << "[IntegrationDQMOffline::" << __func__
...
void ots::IntegrationDQMOffline::summary_calo_fill(
    art::Event const &event, SimpleDQMHistoContainer &hists,
    const bool fill_pulses) {
  ...
  for (auto &digi : *digis) {
    ...
    if (fill_pulses) {
      if (!sipmIDs_.count(sipmID))
        sipmIDs_[sipmID] = sipmIDs_.size() + nCaloSummary_;
      const size_t index = sipmIDs_[sipmID];
      if (index >= hists.histograms.size())
        continue;
...
void ots::IntegrationDQMOffline::summary_trk_fill(
    art::Event const &event, SimpleDQMHistoContainer &hists,
    const bool fill_pulses) {
  ...
  hists.histograms[0]._Hist->Fill(digis->size());
  for (auto &digi : *digis) {
    hists.histograms[1]._Hist->Fill(digi.TDC()[0] * 1.f); // FIXME: Convert to
                                                          // ns
  }
}

void ots::IntegrationDQMOffline::summary_crv_fill(
    art::Event const &event, SimpleDQMHistoContainer &hists,
    const bool fill_pulses) {
...
// void ots::IntegrationDQMOffline::summary_crv_fill(art::Event const& event,
// SimpleDQMHistoContainer& hists, const bool fill_pulses) {
//   if (hists.histograms.size() == 0) {
//     __COUT__ << __func__ << ": No histograms booked" << std::endl;
//     return;
//   }
...
//   if(!crv_decoders_) {
//     __COUT__ << "[IntegrationDQMOffline::" << __func__ << "] No CRV decoders
//     found with tag " << crvDigiTag_.c_str() << std::endl;
//     hists.histograms[0]._Hist->Fill(0);
//     return;
//   }
...
//     art::Handle<mu2e::CrvDigiCollection> crvDigisH;
//     if(!event.getByLabel(crvDigiTag_, crvDigisH)) {
//       __COUT__ << "[IntegrationDQMOffline::" << __func__ << "] No CRV digis
//       found with tag " << crvDigiTag_.c_str() << std::endl;
//       hists.histograms[0]._Hist->Fill(0);
//       return;
//     }
...
//     hists.histograms[0]._Hist->Fill(digis->size());
//     for(auto& digi : *digis) {
//       hists.histograms[1]._Hist->Fill(digi.GetStartTDC()*1.f); //FIXME:
//       Convert to ns
//     }
//   }
// }
...
// STM
void ots::IntegrationDQMOffline::summary_stm_fill(
    art::Event const &event, SimpleDQMHistoContainer &hists,
    const bool fill_pulses) {
  if (hists.histograms.size() == 0) {
    __COUT__ << __func__ << ": No histograms booked" << std::endl;
...
void ots::IntegrationDQMOffline::summary_global_fill(
    art::Event const &event, SimpleDQMHistoContainer &hists) {
  if (hists.histograms.size() == 0) {
    __COUT__ << __func__ << ": No histograms booked" << std::endl;
    return;
  ...
  if (digi_count_index < hists.histograms.size()) {
    ...
    __COUT__ << "[IntegrationDQMOffline::" << __func__
             << "] Histogram list not large enough! Index = "
             << digi_count_index << " size = " << hists.histograms.size()
             << std::endl;
  }
  ...
  if (fillCRV) {
    for (auto &digi : *crvDigis) {
      crv_t0 =
          std::min(crv_t0, digi.GetStartTDC() * 1.f); // FIXME: Convert to ns
    }
  }
  if (fillCRVGR) {
    crv_t0 = 1.e10f;
  ...
  if (fillSTM) {
    for (auto &digi : *stmDigis) {
      stm_t0 =
          std::min(stm_t0, digi.trigTimeOffset() * 1.f); // FIXME: Convert to ns
    }
  }

  // Fill the subdetector time difference
  if (fillTrk && fillCalo)
    hists.histograms[0]._Hist->Fill(trk_t0 - calo_t0);
  if (fillTrk && fillCRV)
    hists.histograms[1]._Hist->Fill(trk_t0 - crv_t0);
  if (fillTrk && fillSTM)
    hists.histograms[2]._Hist->Fill(trk_t0 - stm_t0);
  if (fillCalo && fillCRV)
    hists.histograms[3]._Hist->Fill(calo_t0 - crv_t0);
  if (fillCalo && fillSTM)
    hists.histograms[4]._Hist->Fill(calo_t0 - stm_t0);
  if (fillCRV && fillSTM)
    hists.histograms[5]._Hist->Fill(crv_t0 - stm_t0);
}

// Decode CRV GR data from a vector of decoders
// Returns a vector of CRVGlobalRunInfo objects
std::vector<mu2e::CRVDataDecoder::CRVGlobalRunInfo>
ots::IntegrationDQMOffline::decode_crv(
  ...
  for (size_t iSubEvent = 0; iSubEvent < nSubEvents; ++iSubEvent) {
    ...
    for (size_t iDataBlock = 0; iDataBlock < CRVDataDecoder.block_count();
      ...
      if (block == nullptr)
        continue; // Skip if block

      // Get block header and validate it
      auto header = block->GetHeader();
      if (!header->isValid())
        continue; // Skip if header is invalid
      ...
      if (header->GetPacketCount() > 0) {
        ...
        if (crvRocHeader == nullptr)
          continue; // Skip if ROC header is invalid
