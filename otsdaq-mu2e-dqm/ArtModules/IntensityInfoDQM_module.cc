...
private:
  ...
  IntensityInfoDQMHistoContainer *summary_histos =
      new IntensityInfoDQMHistoContainer();
...
ots::IntensityInfoDQM::IntensityInfoDQM(Parameters const &conf)
    : art::EDAnalyzer(conf), conf_(conf()), port_(conf().port()),
      ...
      histType_(conf().histType()), freqDQM_(conf().freqDQM()),
      diagLevel_(conf().diag()), evtCounter_(0), doOnspillHist_(false),
      doOffspillHist_(false) {
  histSender_ = new HistoSender(address_, port_);

  if (diagLevel_ > 0) {
    __COUT__ << "[IntensityInfoDQM::analyze] DQM for " << histType_[0]
             << std::endl;
  }
...
void ots::IntensityInfoDQM::beginJob() {
  ...
  summary_histos->BookSummaryHistos(tfs, "CAPHRI hits; nCAPHRIHits; Events",
                                    100, 0, 100);
  // caloInfo
  summary_histos->BookSummaryHistos(
      tfs, "IntensityInfo Calo, nHits; nCaloHits; Events/60", 200, 0, 12e3);
  summary_histos->BookSummaryHistos(
      tfs, "IntensityInfo Calo, caloEnergy; E[MeV]; Events/(5 MeV)", 400, 0,
      2e3);

  // tracker info
  summary_histos->BookSummaryHistos(tfs, "IntensityInfo Tracker; nTrkHits", 200,
                                    0, 12e3);
}
...
void ots::IntensityInfoDQM::analyze(art::Event const &event) {
  ...
  auto const caloH =
      event.getValidHandle<mu2e::IntensityInfoCalo>("CaloHitMakerFast");
  const mu2e::IntensityInfoCalo *caloInfos = caloH.product();

  auto const trkH =
      event.getValidHandle<mu2e::IntensityInfoTrackerHits>("TTmakeSH");
...
void ots::IntensityInfoDQM::summary_fill(
    IntensityInfoDQMHistoContainer *histos,
    const mu2e::CaloHitCollection *CAPHRIHits,
    const mu2e::IntensityInfoCalo *CaloInfos,
    const mu2e::IntensityInfoTrackerHits *TrkInfos) {
