...
namespace ots {
  ...
  public:
    ...
    void BookSingleChannel(art::ServiceHandle<art::TFileService> tfs, int mapID, std::string Name, std::string Title, int nBins, float min, float max) {
      ...
      this->map_h1_dt_singlechan[mapID]._Hist = testDir.make<TH1F>(Name.c_str(), Title.c_str(), nBins, min, max);
      this->map_h1_dt_singlechan[mapID]._Hist->GetXaxis()->SetTitle("dt [ns]");
    }
    void BookSingleChannelG(art::ServiceHandle<art::TFileService> tfs, int mapID, std::string Name, std::string Title) {
      TLOG(TLVL_DEBUG) << "Booking single channel graph for sipmID " << mapID;
      map_g_dtevt_singlechan[mapID] = InfoGraph_();
      art::TFileDirectory testDir = tfs->mkdir("SingleChannel");
      this->map_g_dtevt_singlechan[mapID]._Graph = testDir.makeAndRegister<TGraph>(Name.c_str(), Title.c_str());
    ...
    void BookSameBoard(art::ServiceHandle<art::TFileService> tfs, int mapID, std::string Name, std::string Title, int nBins, float min, float max) {
      ...
      this->map_h1_dt_sameboard[mapID]._Hist = testDir.make<TH1F>(Name.c_str(), Title.c_str(), nBins, min, max);
      this->map_h1_dt_sameboard[mapID]._Hist->GetXaxis()->SetTitle("dt [ns]");
    }
    void BookSameBoardG(art::ServiceHandle<art::TFileService> tfs, int mapID, std::string Name, std::string Title) {
      TLOG(TLVL_DEBUG) << "Booking single board graph for sipmID " << mapID;
      map_g_dtevt_sameboard[mapID] = InfoGraph_();
      art::TFileDirectory testDir = tfs->mkdir("SameBoard");
      this->map_g_dtevt_sameboard[mapID]._Graph = testDir.makeAndRegister<TGraph>(Name.c_str(), Title.c_str());
    ...
    void BookDiffBoard(art::ServiceHandle<art::TFileService> tfs, int mapID, std::string Name, std::string Title, int nBins, float min, float max) {
      ...
      this->map_h1_dt_diffboard[mapID]._Hist = testDir.make<TH1F>(Name.c_str(), Title.c_str(), nBins, min, max);
      this->map_h1_dt_diffboard[mapID]._Hist->GetXaxis()->SetTitle("dt [ns]");
    }
    void BookDiffBoardG(art::ServiceHandle<art::TFileService> tfs, int mapID, std::string Name, std::string Title) {
      TLOG(TLVL_DEBUG) << "Booking diff board graph for sipmID " << mapID;
      map_g_dtevt_diffboard[mapID] = InfoGraph_();
      art::TFileDirectory testDir = tfs->mkdir("DiffBoard");
      this->map_g_dtevt_diffboard[mapID]._Graph = testDir.makeAndRegister<TGraph>(Name.c_str(), Title.c_str());
