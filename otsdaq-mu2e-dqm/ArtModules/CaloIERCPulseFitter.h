...
class CaloIERCPulseFitter
{
  public:
	...
	CaloIERCPulseFitter(const char* file_name, const int diag = 0)
	    : _diag(diag), _f(nullptr), _spline(nullptr), _f_spline(nullptr)
	{
		...
		if(!_f)
		{
			std::cout << "[CaloIERCPulseFitter::" << __func__ << "] No file " << file_name
			          << " found\n";
			return;
		}
		...
		if(!_spline)
		{
			std::cout << "[CaloIERCPulseFitter::" << __func__
			          << "] No template found in spline file\n";
			return;
		}
		_f_spline = new TF1(
		    "f_spline",
		    [this](double* x, double* par) {
			    return par[0] * this->_spline->Eval(x[0] - par[1]) + par[2];
		    },
		    3.,
		    25.,
		    3);
	...
	FitResult_t fit_pulse(const mu2e::CaloDigi* digi)
	{
		...
		// Fit the waveform
		_f_spline->SetParameters(
		    3850.,
		    0.,
		    0.);  // initialize the default parameters FIXME: Make these fields
		// const int fitStatus = gadc->Fit("f_spline","QRN"); // fit the waveform
		const auto result =
		    gadc->Fit("f_spline", (_diag > 1) ? "RNS" : "QRNS");  // fit the waveform
		if(result >= 0)
		{
			...
			if(_diag > 1)
				std::cout << "[CaloIERCPulseFitter::" << __func__
				          << "] Fit result: time = " << fit_result._time << " +- "
				          << fit_result._time_unc << " status = " << fit_result._status
				          << std::endl;
		}
		else
		{
			if(_diag > 0)
				std::cout << "[CaloIERCPulseFitter::" << __func__ << "] Fit failed!\n";
